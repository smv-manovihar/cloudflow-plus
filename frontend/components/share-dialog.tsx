...
export function ShareDialog({
  ...
  const shareUrl = useMemo(() => {
    ...
    return `${typeof window !== "undefined" ? window.location.origin : "https://example.com"}/download/${id}`
  }, [item])
      ...
      <DialogContent aria-describedby="share-desc">
        <DialogHeader>
          ...
          <DialogDescription id="share-desc">
            Create a public link and optional QR code to share this {item?.type === "folder" ? "folder" : "file"}.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4">
          <div className="flex items-center justify-between rounded-md border p-3">
            ...
            <Switch checked={enabled} onCheckedChange={setEnabled} aria-label="Enable public link" />
          </div>

          <div className="grid gap-2">
            <Label htmlFor="share-url">Shareable link</Label>
            <div className="flex items-center gap-2">
              <Input id="share-url" value={enabled ? shareUrl : ""} readOnly placeholder="Disabled" />
              <Button
                variant="secondary"
                onClick={() => {
                  if (!enabled) return
                  navigator.clipboard.writeText(shareUrl)
                  toast({ title: "Copied link", description: "The public link was copied to your clipboard." })
                }}
                disabled={!enabled}
              >
                Copy
          ...
          <div className="grid gap-2">
            ...
            <div className="flex items-center justify-center rounded-md border p-4 min-h-[220px]">
              {enabled && qrDataUrl ? (
                ...
                <p className="text-sm text-muted-foreground">Enable the link to generate a QR code.</p>
              )}
            </div>
          </div>
        </div>
